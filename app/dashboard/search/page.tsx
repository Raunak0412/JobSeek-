        ...
        <main className="flex-1 space-y-6 p-4 lg:p-6">
          <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div>
                <Badge className="rounded-full border-red-400/20 bg-red-400/10 text-red-100">Global dashboard search</Badge>
            ...
            <CardContent>
              <Tabs value={filter} onValueChange={(value) => setFilter(value as WorkspaceSearchType | "all")} className="space-y-4">
                <TabsList className="h-auto flex-wrap rounded-full border border-white/10 bg-white/5 p-1">
                  ...
                  <TabsTrigger value="page" className="rounded-full px-4 text-slate-200 data-[state=active]:bg-red-400/15 data-[state=active]:text-red-100">
                    Pages ({groupedCounts.page})
                  </TabsTrigger>
                  <TabsTrigger value="candidate" className="rounded-full px-4 text-slate-200 data-[state=active]:bg-red-400/15 data-[state=active]:text-red-100">
                    Candidates ({groupedCounts.candidate})
                  </TabsTrigger>
                  <TabsTrigger value="job" className="rounded-full px-4 text-slate-200 data-[state=active]:bg-red-400/15 data-[state=active]:text-red-100">
                    Vacancies ({groupedCounts.job})
                  </TabsTrigger>
                  <TabsTrigger value="application" className="rounded-full px-4 text-slate-200 data-[state=active]:bg-red-400/15 data-[state=active]:text-red-100">
                    Applications ({groupedCounts.application})
                  </TabsTrigger>
                  <TabsTrigger value="notification" className="rounded-full px-4 text-slate-200 data-[state=active]:bg-red-400/15 data-[state=active]:text-red-100">
                    ...
                    {filtered.length ? (
                      filtered.map((item) => (
                        <button
                          ...
                          className="w-full rounded-2xl border border-white/10 bg-white/5 p-4 text-left transition hover:border-red-300/35 hover:bg-white/10"
                        >
                          ...
                          <span className="mt-3 inline-flex items-center text-xs text-red-200">
                            Open result
          ...
          <Card className="rounded-[1.75rem] border-white/10 bg-[#1b0b0b]">
            <CardContent className="flex items-start gap-3 p-5 text-sm leading-6 text-slate-300">
              ...
              <p>
                ...
                <Link href="/dashboard/notifications" className="ml-1 text-red-200 underline-offset-4 hover:underline">
